...
static bool brcmf_force_cold_resume;
module_param_named(force_cold_resume, brcmf_force_cold_resume, bool, 0644);
MODULE_PARM_DESC(force_cold_resume,
		 "Force cold resume (full recovery) path on every resume for testing");
...
/*
 * Deep sleep state IDs — firmware DS state machine.
 * Reconstructed from macOS changeSleepState() decompilation and
 * firmware DS_STATE_* string table at 0x0021cfb1.
 */
#define BRCMF_DS_STATE_ACTIVE			5
#define BRCMF_DS_STATE_HOST_SLEEP_PEND_TOP	6
#define BRCMF_DS_STATE_HOST_SLEEP_PEND_BOT	7
#define BRCMF_DS_STATE_DEVICE_SLEEP_WAIT	8
#define BRCMF_DS_STATE_DEVICE_SLEEP		9
#define BRCMF_DS_STATE_DEVICE_ACTIVE_WAIT	10
#define BRCMF_DS_STATE_HOST_SLEEP		11
#define BRCMF_DS_STATE_D3_ENTER_WAIT		12
#define BRCMF_DS_STATE_D3_ENTERED		13

/* Timeout for device to exit deep sleep after doorbell wake (ms) */
#define BRCMF_DS_EXIT_TIMEOUT			msecs_to_jiffies(1000)
/* Delay after D0_INFORM for firmware re-initialization (ms) */
#define BRCMF_D0_SETTLE_TIME			200
...
struct brcmf_pciedev_info {
	...
	bool irq_allocated;
	bool wowl_enabled;
	/* Deep sleep state tracking — see pciedev_ds_ack_host_sleep */
	u8 ds_state;
	bool ds_d3_informed;
...
static void brcmf_pcie_handle_mb_data(struct brcmf_pciedev_info *devinfo)
{
	...
	if (dtoh_mb_data & BRCMF_D2H_DEV_DS_ENTER_REQ) {
		pr_info("brcmfmac: D2H mailbox: DEEP SLEEP REQ, ds_state %u -> %u\n",
			   devinfo->ds_state, BRCMF_DS_STATE_DEVICE_SLEEP);
		devinfo->ds_state = BRCMF_DS_STATE_DEVICE_SLEEP;
		brcmf_pcie_send_mb_data(devinfo, BRCMF_H2D_HOST_DS_ACK);
		pr_info("brcmfmac: D2H mailbox: sent DEEP SLEEP ACK\n");
	}
	if (dtoh_mb_data & BRCMF_D2H_DEV_DS_EXIT_NOTE) {
		pr_info("brcmfmac: D2H mailbox: DEEP SLEEP EXIT, ds_state %u -> %u\n",
			   devinfo->ds_state, BRCMF_DS_STATE_ACTIVE);
		devinfo->ds_state = BRCMF_DS_STATE_ACTIVE;
		wake_up(&devinfo->ds_exit_wait);
	}
	if (dtoh_mb_data & BRCMF_D2H_DEV_D3_ACK) {
		pr_info("brcmfmac: D2H mailbox: D3 ACK received, ds_state %u -> %u\n",
			   devinfo->ds_state, BRCMF_DS_STATE_D3_ENTERED);
		devinfo->ds_state = BRCMF_DS_STATE_D3_ENTERED;
		devinfo->mbdata_completed = true;
		wake_up(&devinfo->mbdata_resp_wait);
	}
	if (dtoh_mb_data & BRCMF_D2H_DEV_FWHALT) {
		pr_info("brcmfmac: D2H mailbox: FW HALT\n");
		if (devinfo->recovery_in_progress) {
			pr_info("brcmfmac: D2H mailbox: FW HALT during recovery, suppressing\n");
		} else {
			     ...
			     struct brcmf_fw_request *fwreq)
{
	...
	if (devinfo->recovery_in_progress) {
		pr_info("brcmfmac: recovery complete, device operational\n");
		devinfo->recovery_in_progress = false;
	}
...
fail:
	...
	brcmf_pcie_bus_console_read(devinfo, true);
	if (devinfo->recovery_in_progress) {
		brcmf_err(bus, "Dongle setup failed during recovery, giving up\n");
		devinfo->recovery_in_progress = false;
	} else {
...
static int brcmf_pcie_pm_enter_D3(struct device *dev)
{
	...
	pr_info("brcmfmac: enter_D3: starting, ds_state=%u, pcie_state=%d\n",
		   devinfo->ds_state, devinfo->state);

	/* If device is already down (e.g. recovery work pending from a
	 * previous failed resume), let PM proceed without touching HW.
	 */
	if (devinfo->state == BRCMFMAC_PCIE_STATE_DOWN) {
		pr_info("brcmfmac: enter_D3: device already down (recovery pending?), skipping D3 handshake\n");
		return 0;
	}
	...
	/*
	 * Step 1: If device is in deep sleep, wake it first by hitting the
	 * doorbell. Firmware expects to be in active state before
	 * processing D3_INFORM — see pciedev_ds_ack_host_sleep.
	 */
	if (devinfo->ds_state == BRCMF_DS_STATE_DEVICE_SLEEP ||
	    devinfo->ds_state == BRCMF_DS_STATE_DEVICE_SLEEP_WAIT) {
		pr_info("brcmfmac: enter_D3: device in deep sleep (state %u), writing doorbell to wake\n",
			   devinfo->ds_state);
		brcmf_pcie_write_reg32(devinfo,
				       devinfo->reginfo->h2d_mailbox_0, 1);

		if (!wait_event_timeout(devinfo->ds_exit_wait,
				devinfo->ds_state == BRCMF_DS_STATE_ACTIVE,
				BRCMF_DS_EXIT_TIMEOUT)) {
			brcmf_err(bus, "enter_D3: DS exit timeout (state still %u), proceeding anyway\n",
				  devinfo->ds_state);
		} else {
			pr_info("brcmfmac: enter_D3: device woke from deep sleep, ds_state=%u\n",
				   devinfo->ds_state);
		}
	}

	/* Step 2: Mark bus as down to stop new TX submissions */
	pr_info("brcmfmac: enter_D3: setting bus state DOWN\n");
	brcmf_bus_change_state(bus, BRCMF_BUS_DOWN);

	/* Step 3: Disable interrupts to prevent ring events during transition */
	pr_info("brcmfmac: enter_D3: disabling interrupts\n");
	brcmf_pcie_intr_disable(devinfo);

	/* Step 4: Send D3_INFORM to firmware */
	devinfo->ds_state = BRCMF_DS_STATE_HOST_SLEEP;
	devinfo->mbdata_completed = false;
	devinfo->ds_d3_informed = true;
	pr_info("brcmfmac: enter_D3: sending D3_INFORM (H2D mailbox=0x%x), ds_state=%u\n",
		   BRCMF_H2D_HOST_D3_INFORM, devinfo->ds_state);
	brcmf_pcie_send_mb_data(devinfo, BRCMF_H2D_HOST_D3_INFORM);

	/*
	 * Step 5: Re-enable interrupts briefly so we can receive the D3 ACK
	 * from the firmware's D2H mailbox interrupt.
	 */
	brcmf_pcie_intr_enable(devinfo);

	/* Step 6: Wait for D3 ACK from firmware (2s timeout) */
	pr_info("brcmfmac: enter_D3: waiting for D3_ACK (timeout=%ums)\n",
		   jiffies_to_msecs(BRCMF_PCIE_MBDATA_TIMEOUT));
	wait_event_timeout(devinfo->mbdata_resp_wait, devinfo->mbdata_completed,
			   BRCMF_PCIE_MBDATA_TIMEOUT);
	if (!devinfo->mbdata_completed) {
		brcmf_err(bus, "enter_D3: TIMEOUT waiting for D3_ACK from firmware (ds_state=%u)\n",
			  devinfo->ds_state);
		devinfo->ds_d3_informed = false;
		devinfo->ds_state = BRCMF_DS_STATE_ACTIVE;
		brcmf_pcie_intr_enable(devinfo);
	...
	/* Step 7: D3 ACK received — disable interrupts for power down */
	pr_info("brcmfmac: enter_D3: D3_ACK received, disabling interrupts for power down\n");
	brcmf_pcie_intr_disable(devinfo);
	devinfo->state = BRCMFMAC_PCIE_STATE_DOWN;

	pr_info("brcmfmac: enter_D3: complete, device entering D3\n");
	return 0;
}
...
static int brcmf_pcie_pm_leave_D3(struct device *dev)
{
	...
	pr_info("brcmfmac: leave_D3: starting, ds_d3_informed=%d, ds_state=%u\n",
		   devinfo->ds_d3_informed, devinfo->ds_state);

	if (brcmf_force_cold_resume) {
		pr_info("brcmfmac: leave_D3: force_cold_resume enabled, skipping hot resume\n");
		goto cleanup;
	}

	/*
	 * Hot resume path: D3 inform was properly sent and ACKed.
	 *
	 * The intmask register is zero (interrupts disabled during D3),
	 * so check TCM shared memory accessibility as the primary
	 * liveness test instead.
	 */
	if (devinfo->ds_d3_informed) {
		/* Step 1: Verify TCM is accessible (device still powered) */
		val = brcmf_pcie_read_tcm32(devinfo,
					    devinfo->shared.tcm_base_address);
		pr_info("brcmfmac: leave_D3: TCM probe @0x%x = 0x%08x\n",
			   devinfo->shared.tcm_base_address, val);
		if (val == 0 || val == 0xffffffff) {
			brcmf_err(bus, "leave_D3: TCM inaccessible (0x%08x), falling back to cold resume\n",
				  val);
			goto cleanup;
		}

		/* Step 2: Select PCIe core for register access */
		pr_info("brcmfmac: leave_D3: hot resume, selecting PCIe core\n");
		brcmf_pcie_select_core(devinfo, BCMA_CORE_PCIE2);

		/* Step 3: Send D0_INFORM first, then hostready — firmware
		 * expects this order. See pciedev_handle_d0_enter_bm.
		 */
		pr_info("brcmfmac: leave_D3: sending D0_INFORM (H2D mailbox=0x%x)\n",
			   BRCMF_H2D_HOST_D0_INFORM);
		if (brcmf_pcie_send_mb_data(devinfo,
					    BRCMF_H2D_HOST_D0_INFORM)) {
			brcmf_err(bus, "leave_D3: failed to send D0_INFORM, falling back to cold resume\n");
			goto cleanup;
		}

		/* Step 4: Re-enable interrupts and send hostready doorbell */
		pr_info("brcmfmac: leave_D3: enabling interrupts\n");
		brcmf_pcie_intr_enable(devinfo);

		pr_info("brcmfmac: leave_D3: sending hostready doorbell\n");
		brcmf_pcie_hostready(devinfo);

		/*
		 * Step 5: Wait for firmware to process D0_INFORM and
		 * re-initialize (ring setup, counter restore, timer restart).
		 * See pciedev_handle_d0_enter_bm / pciedev_handle_hostready_intr.
		 */
		pr_info("brcmfmac: leave_D3: waiting %ums for firmware re-init\n",
			   BRCMF_D0_SETTLE_TIME);
		msleep(BRCMF_D0_SETTLE_TIME);

		/* Step 6: Post-resume sanity — verify TCM is still alive
		 * after firmware re-init (catches firmware crash during resume).
		 */
		val = brcmf_pcie_read_tcm32(devinfo,
					    devinfo->shared.tcm_base_address);
		pr_info("brcmfmac: leave_D3: post-settle TCM probe = 0x%08x\n",
			   val);
		if (val == 0 || val == 0xffffffff) {
			brcmf_err(bus, "leave_D3: post-resume sanity failed (TCM=0x%08x), firmware may have crashed during re-init\n",
				  val);
			goto cleanup;
		}

		/* Step 7: Everything looks good — bring bus back up */
		devinfo->state = BRCMFMAC_PCIE_STATE_UP;
		devinfo->ds_state = BRCMF_DS_STATE_ACTIVE;
		devinfo->ds_d3_informed = false;
		brcmf_bus_change_state(bus, BRCMF_BUS_UP);
		...
		pr_info("brcmfmac: leave_D3: hot resume complete, bus UP\n");
		return 0;
	}

	/*
	 * Legacy resume path: No D3 inform was sent, fall back to
	 * original intmask-based check.
	 */
	val = brcmf_pcie_read_reg32(devinfo, devinfo->reginfo->intmask);
	pr_info("brcmfmac: leave_D3: legacy path, intmask=0x%08x\n", val);
	if (val != 0) {
		pr_info("brcmfmac: leave_D3: legacy hot resume, sending D0_INFORM\n");
		if (brcmf_pcie_send_mb_data(devinfo,
					    BRCMF_H2D_HOST_D0_INFORM))
			goto cleanup;
		devinfo->state = BRCMFMAC_PCIE_STATE_UP;
		...
		brcmf_pcie_hostready(devinfo);
		brcmf_pcie_fwcon_timer(devinfo, true);
		pr_info("brcmfmac: leave_D3: legacy hot resume complete, bus UP\n");
		return 0;
	}

cleanup:
	/*
	 * Device is unresponsive after D3 exit. Rather than calling
	 * brcmf_pcie_remove() + brcmf_pcie_probe() here — which frees
	 * devinfo/bus (use-after-free risk) and calls pci_disable_device()
	 * inside a PM callback (unsafe) — use the existing crash recovery
	 * infrastructure: brcmf_fw_crashed() schedules brcmf_pcie_reset()
	 * on a workqueue, which safely tears down rings/IRQ and reloads
	 * firmware without destroying PCI resource mappings.
	 */
	pr_info("brcmfmac: leave_D3: device unresponsive, scheduling deferred reset recovery\n");
	devinfo->ds_d3_informed = false;
	devinfo->ds_state = BRCMF_DS_STATE_ACTIVE;
	devinfo->state = BRCMFMAC_PCIE_STATE_DOWN;
	/* Ensure interrupts are off — the legacy path may arrive here
	 * with interrupts still enabled if D0_INFORM send failed.
	 */
	brcmf_pcie_intr_disable(devinfo);
	brcmf_bus_change_state(bus, BRCMF_BUS_DOWN);
	devinfo->recovery_in_progress = true;
	brcmf_fw_crashed(&devinfo->pdev->dev);
	pr_info("brcmfmac: leave_D3: reset work scheduled, WiFi will recover asynchronously\n");
	return 0;
}
